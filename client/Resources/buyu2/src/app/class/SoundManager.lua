...
function SoundManager:PlayBackMusic( )
    ...
    local path = string.format("buyu2/res/sounds/bgm/bgm%d.mp3", self.bg_music_)
...
function SoundManager:PlayGameEffect( game_effect )
    ...
    if game_effect == 0 then
        path = "buyu2/res/sounds/effect/cannonSwitch.wav"
    elseif game_effect == 1 then
        path = "buyu2/res/sounds/effect/casting.wav"
    elseif game_effect == 2 then
        path = "buyu2/res/sounds/effect/catch.wav"
    elseif game_effect == 3 then
        path = "buyu2/res/sounds/effect/fire.wav"
    elseif game_effect == 4 then
        path = "buyu2/res/sounds/effect/superarm.wav"
    elseif game_effect == 5 then
        path = "buyu2/res/sounds/effect/bingo.wav"
    elseif game_effect == 6 then
        path = "buyu2/res/sounds/effect/wave.wav"
    end
...
function SoundManager:PlayFishEffect( fish_kind )
    if fish_kind >= 11 and fish_kind <= 16 then
        ...
        if fish_kind == 16 then
            path = string.format("buyu2/res/sounds/effect/fish%d_%d.wav", fish_kind+1, math.random(0, 65535)%3 + 1)
        else
            path = string.format("buyu2/res/sounds/effect/fish%d_%d.wav", fish_kind+1, math.random(0, 65535)%2 + 1)
        end
        AudioEngine.playEffect(path)
    end
end
