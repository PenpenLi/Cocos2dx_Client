...
function GameScene:ctor()
 ...
 -- cc.exports.delayFishId = -1
  ...
  self.labletext = cc.Label:createWithTTF("adadad", "fullmain/res/fonts/FZY4JW.ttf", 25, cc.size(900, 0))
...
function GameScene:initSurface()
  ...
  for k, v in pairs(tableUser) do
    ...
    if pd.playerId == global.g_mainPlayer:getPlayerId() then
      ...
      local px = self.paotable[self.playnum+1]:getPositionX()
      local py = self.paotable[self.playnum+1]:getPositionY()
      self.paoPoint = cc.p(px,py)
...
function GameScene:onAutoExitHandler()
  exit_buyu2(GAME_GUIDES.GUIDE_HALL)
end
...
function GameScene:onMessage(param)
  ...
  if true then --string.find(str,"摇钱树") then
    local dt = 1
...
function GameScene:layer()
  ...
  self.freetimer:setPosition(645,5)
