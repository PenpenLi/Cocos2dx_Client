...
function onFishTrace( buffObj )
	local fish_trace_count = buffObj:getLength()/64 --数目
	local param = {}
	...
	for i=1,fish_trace_count do
		...
		for i=1,6 do
			...
			fishtrace.init_pos[i] = cc.p(point1.x*hscale,point1.y*vscale)
		end
...
function onFishLine( buffObj )
	...
	global.g_gameDispatcher:sendMessage(GAME_MESSAGE_BUYU_FISH_LINE, param)
end
...
--	cclog("FishHandler:onUserFire")
	...
	param.bullet_multiple = buffObj:readInt() --正数,子弹倍率
...
--	cclog("FishHandler:onCatchFish")
	...
	param.bullet_ion = buffObj:readInt() --获得特殊子弹 1双倍子弹 2穿甲弹 3免费子弹
...
function onFireSpeedFlag( buffObj )
	...
	local param = {}
	param.player_fire_speed = buffObj:readInt()
	--cclog("player_fire_speed = %d",player_fire_speed)
	global.g_gameDispatcher:sendMessage(GAME_MESSAGE_USER_FIRE_SPEED, param)
end
...
--	cclog("FishHandler:OnGameSceneMessage")
	...
	param.fish_score = {}
	for i=1,8 do
		param.fish_score[i] = buffObj:readInt64()
	end
	param.coin_score = {}
	for i=1,8 do
		param.coin_score[i] = buffObj:readInt64()
	end
