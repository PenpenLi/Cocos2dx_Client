ui_table168_h_t = class("ui_table168_h_t", NodeBase)

function ui_table168_h_t:ctor(tableId)
	ui_table168_h_t.super.ctor(self)

	self.tableId_ = tableId

	local serverId = global.g_mainPlayer:getCurrentRoom()
	local root = ccs.GUIReader:getInstance():widgetFromJsonFile("fullmain/res/json/selectLevel/horizontal/ui_main_h_machine_168.json")
	self:addChild(root)
...
function ui_table168_h_t:initSurface()
...
function ui_table168_h_t:stopAllAniChair()
...
function ui_table168_h_t:randomAniChair()
...
function ui_table168_h_t:setPersonVisible(bVisible, chairId)
	local person = self.players_[chairId]
	person:setVisible(bVisible)
end

function ui_table168_h_t:setArrowVisible(bVisible, chairId)
...
function ui_table168_h_t:onTouchChair(sender)
...
function ui_table168_h_t:userLeave(chairId)
...
function ui_table168_h_t:userSitdown(chairId)
