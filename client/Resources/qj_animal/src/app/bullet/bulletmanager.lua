...
function bulletmanager:init()
 ...
 --  spriteFrame:addSpriteFrames("qj_animal/res/game_res/bulletEx.plist");
   --local i=0;
  -- for i=0,20,1 do
   --  cc.exports.g_offsetBullet_InitFlag[i]=0;
   --end
end
function bulletmanager:FreeAllBullet()
	...
	if (self:getChildrenCount() > 0) then 
       ...
       local i=0;
		for i=#pChildren,0,-1 do
				...
				if (bullet ) then 
                          if(bullet:getBomFlag()>0)  then self:removeChild(bullet);  
                          else 
                             local t_pos=bullet:get_mov_pos();
                             if(t_pos.x<-20 or t_pos.y<-30 or t_pos.x>1310 or t_pos.y>750) then self:removeChild(bullet); end
...
function bulletmanager:Fire( src_x_pos,  src_y_pos,  angle,  bullet_kind,  bullet_id,  bullet_mulriple,  firer_chair_id, bullet_speed,  net_radius,  lock_fishid,andro_chair_id)
	if (firer_chair_id >= GAME_PLAYER or firer_chair_id<0) then  return 0; end
   ...
   -- src_x_pos,  src_y_pos,  angle,  bullet_kind,  bullet_id,  bullet_mulriple,  firer_chair_id, bullet_speed,  net_radius,  lock_fishid);
    ...
    if(cc.exports.g_game_player_bullet_num_list[firer_chair_id]>40)then  return 0; end
	local bullet =cc.exports.Bullet.new(bullet_kind, bullet_id, bullet_mulriple, firer_chair_id, net_radius, cc.p(src_x_pos, src_y_pos), bullet_speed, angle, lock_fishid,andro_chair_id);
...
--	end
 local  tem_me_bulletNum = 0;
      ...
      cclog("bulletmanager:GetBulletNum tem_me_bulletNum=%d",tem_me_bulletNum);
