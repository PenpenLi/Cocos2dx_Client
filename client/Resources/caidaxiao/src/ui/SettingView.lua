...
function SettingView:ctor()
	...
	local root = ccs.GUIReader:getInstance():widgetFromJsonFile("main/res/json/ui_main_setting.json")
	...
	local volumeMusic = global.g_mainPlayer:getLocalData("caidaxiao_music", 1)
	self.sliderSound_:setPercent(volumeMusic * 100)

	local volumeEffect = global.g_mainPlayer:getLocalData("caidaxiao_effect", 1)
...
function SettingView:onClose()
	...
	global.g_mainPlayer:setLocalData("caidaxiao_effect", volumeEffect)
	global.g_mainPlayer:setLocalData("caidaxiao_music", volumeMusic)
