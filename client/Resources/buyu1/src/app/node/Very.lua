...
function Very:ctor(a,sum,android_chairid)
  ...
  self.fishnet = 0
  --cclog("self.playID = %d,a = %d",self.playID,a)
  ...
  if a >= 0 and a <= 3 then
    local cc = a + 1
    file = "buyu1/res/images/bullet/".."bullet"..cc.."_norm"..pon..".png"
  elseif a >= 4 and a <= 7 then
    local dd = a - 3
    file = "buyu1/res/images/bullet/".."bullet"..dd.."_ion.png"
  end


  local textur = cc.Director:getInstance():getTextureCache():addImage(file)
  local actab = {cc.rect(0,0,textur:getContentSize().width/2,textur:getContentSize().height),cc.rect(textur:getContentSize().width/2,0,textur:getContentSize().width/2,textur:getContentSize().height)}
	self.spp = Action:creatsp(file,actab,0.18)
	self:addChild(self.spp)
  self.spp:setScale(1.2)
	self:enableNodeEvents(true)]]
  ...
  local width = self.spp:getContentSize().width * 0.8
  local height = self.spp:getContentSize().height * 0.8
