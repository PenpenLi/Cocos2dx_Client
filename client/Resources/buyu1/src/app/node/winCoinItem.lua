...
function winCoinItem:ctor(score,number,bgColor,index,id)
    ...
    self.numBg = nil
    self.timerdt = 0
    self.sp = cc.Sprite:createWithSpriteFrameName("XY_itemCoin_f00.png")
    ...
    local function CoinMoveEnd()
        ...
        if self.bgColor == 1 then
            self.numBg = cc.Sprite:createWithSpriteFrameName("XY_G0"..len..".png")
        else
            self.numBg = cc.Sprite:createWithSpriteFrameName("XY_R0"..len..".png")
        end
        self.numBg:setPosition(cc.p(0,5*self.num - 5))
        self.numBg:setAnchorPoint(cc.p(0.5,0))
        self:addChild(self.numBg)

        self.numTxt = Digital:create("0","buyu1/res/fishui/item_shuzi.png",9,18,string.byte("0"))
...
function winCoinItem:AddItem()
    if  self.cnt < self.num  then
        local item = cc.Sprite:createWithSpriteFrameName("XY_itemCoin.png")
