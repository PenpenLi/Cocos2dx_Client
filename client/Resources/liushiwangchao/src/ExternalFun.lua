...
--加载界面根节点，设置缩放达到适配 苹果手机iPhone分辨率为1335*750
function ExternalFun.loadRootCSB( csbFile, parent )
  -- local winsize= cc.Director:getInstance():getWinSize()
   -- yl.WIDTH=winsize.width;
   -- yl.HEIGHT=winsize.height;
    cclog("ExternalFun.loadRootCSB csbFile=%s",csbFile);
    ...
    cclog("ExternalFun.loadRootCSB csbFile=%s 11",csbFile);
	local csbnode = cc.CSLoader:createNode(csbFile);
	rootlayer:addChild(csbnode);
    cclog("ExternalFun.loadRootCSB csbFile=%s 12",csbFile);
...
function ExternalFun.formatScoreText(score)
	local scorestr =string.format("%d",score);-- ExternalFun.formatScore(score)
    --[[
	if score < 10000 then
		return scorestr
	end
