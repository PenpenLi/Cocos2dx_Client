...
local GameModel = class("GameModel", function() return cc.Scene:create() end)
-- 初始化界面
function GameModel:ctor()
    ExternalFun.registerNodeEvent(self)
    self._scene =self;-- scene
	self._gameFrame =nil;-- frameEngine
    --设置搜索路径
   ...
   -- self:setSearchPath()
    ...
    --同步结构体 
    self.m_MeItem =global.g_mainPlayer:getRoomPlayer(global.g_mainPlayer.playerId_);
    self.m_MeItem.dwUserID= self.m_MeItem.playerId;
    self.m_MeItem.wTableID=self.m_MeItem.tableId;
    self.m_MeItem.wChairID=self.m_MeItem.chairId;
    self.m_MeItem.cbUserStatus=self.m_MeItem.status
     self.m_MeItem.szNickName=self.m_MeItem.name
...
function GameModel:GetMeChairID()
     self.g_me_chair_id=self.m_MeItem.wChairID;
    return   self.g_me_chair_id;--self._gameFrame:GetChairID()
   -- return self._gameFrame:GetChairID()
end

-- 获取自己桌子
function GameModel:GetMeTableID()
   return self.m_MeItem.wTableID
  --return global.g_mainPlayer:getCurrentTableId();--self._gameFrame:GetTableID()
end

-- 获取自己
function GameModel:GetMeUserItem()    
     self.m_MeItem.lScore= self.lscore;   
     return self.m_MeItem;--self._gameFrame:GetMeUserItem()
...
function GameModel:IsValidViewID( viewId )
    --local nChairCount = self._gameFrame:GetChairCount()
    return (viewId > 0) and (viewId <= 100)
end
