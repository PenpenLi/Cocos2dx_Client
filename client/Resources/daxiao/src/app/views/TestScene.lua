...
function TestScene:ctor()
 ...
 -- cc.exports.delayFishId = -1
  ...
  self.labletext = cc.Label:createWithTTF("adadad", "fullmain/res/fonts/FZY4JW.ttf", 25)
  self.labletext:setPosition(cc.p(winwidth/2,winheight/2))
  self:addChild(self.labletext,100)

  self.labletext:setVisible(false)

  self.bsckome = cc.Sprite:create("buyu2/res/bacckds.png")
  ...
  local function onCreateFish(fishkind, fishid, trace)
    --print("onCreateFish", fishkind, fishid, trace)
...
function TestScene:initBackKey( )
...
function TestScene:initKeyPressed()
...
function TestScene:keyboardPressed(keyCode, event)
  --print(keyCode)
  if keyCode == 26 then
    self:keySetRota(1)
  ...
  elseif keyCode == 51 then  --倍率
    ...
    running_scene.Rate:setVisible(self.isShowBL)
  end
end

function TestScene:keyboardReleased(keycode, event)
...
function TestScene:keySetRota(keyDir)  --1=l,2=r
...
function TestScene:ShakeScreen()
  self.offset_.x = 0
  self.offset_.y = 0
...
function TestScene:UpdateShakeScreen( delta_time )
  ...
  self.bg:setPosition(cc.p(winwidth/2+self.offset_.x,winheight/2+self.offset_.y)) 
  if self.shake_screen_ == false then
     self.bg:setPosition(cc.p(winwidth/2,winheight/2))
  end
end

function TestScene:decUnitBet()
...
function TestScene:onUserFireSpeed(param) -- 发炮间隔,param.player_fire_speed是毫秒
  self.DiTimer = param.player_fire_speed / 1000.0
end

function TestScene:initSurface()
...
function TestScene:onContactBegin(contact)
...
function TestScene:onContactSolve(bodyA, bodyB)
  ...
  if bullet.playID == self.playnum or bullet:getandroid()==self.playnum then
    local param = 0
    if fish.fishKind == 22 or fish.fishKind == 23 or (fish.fishKind >= 27 and fish.fishKind<= 36) then
        ...
        elseif fish.fishKind >= 27 and fish.fishKind<= 36 then
...
function TestScene:onRoomUserLeave(playerId, tableId, chairId, status)
  --有人离开
...
function TestScene:onRoomUserFree(playerId, tableId, chairId, status)
  --有人离开
  local currentTableId = global.g_mainPlayer:getCurrentTableId()
  if tableId == currentTableId and tableId >= 0 and chairId >= 0 then
    --我桌子上有人离开
...
function TestScene:onRoomUserPlay(playerId, tableId, chairId, status)
  --有人进来玩
  local currentTableId = global.g_mainPlayer:getCurrentTableId()
  print("我的桌子id:", currentTableId, "新人的桌子id:", tableId)
  print(playerId,tableId,chairId)
  if tableId == currentTableId and tableId >= 0 and chairId >= 0 then
    --我桌子上有人来玩
...
function TestScene:initMsgHandler()
  -- 后台返回前台
...
function TestScene:removeMsgHandler()
  -- 后台返回前台
...
function TestScene:onGameServerConnectLost()
  print("掉线")
  actor = false
  cc.Director:getInstance():getScheduler():unscheduleScriptEntry(self.schedulerID)
end

function TestScene:onMatchTop(param)
  ...
  if self.playnum >= 0 then
    self.dicscore[self.playnum+1]:setString(tostring(param.My_TopNum))
  end
end

function TestScene:onRedEnvelopeFlag(param)
  ...
  local strd = "距离下一个红包还有："..timh..":"..timm..":"..tims..",红包剩余数量："..timl
  --print(strd,"======================")
  self.hobaotext:setString(strd)
end

function TestScene:onRedEnvelopeSend(param)
  --print("=============asdg12")
...
function TestScene:onEnterForeGround( )
  self:CloseScene()
  self.bolFire = true
end

--配置信息的接入
function TestScene:onGameConfig(param)
  self.fishSpeed = {}
...
function TestScene:onFishLine(param)
  for i = 1,#param do
    ...
    lineFishID[i] = config.fish_id
    --cclog("config.fish_id = %d",config.fish_id)
  end
end

--服务器穿的鱼的创建
function TestScene:onFishTrace(param)
  for i = 1,#param do
    local config = param[i]
    if config.fish_kind>=37 then
      return
    end
    self.factory_:createFish(config.fish_kind, config.fish_id, self.fishSpeed[config.fish_kind], config.init_pos[1].x, config.init_pos[2].x, config.init_pos[3].x,
      config.init_pos[1].y, config.init_pos[2].y, config.init_pos[3].y, config.init_count, config.trace_type)
...
--显示其他场打的鱼的分数
function TestScene:onMessage(param)
  local str = param.message
  if string.find(str,"摇钱树") then
    local dt = 1
...
--服务器游戏分数接入
function TestScene:onExchangeFishscore(param)
...
--同步其他玩家分数
function TestScene:onGFScene(param)
...
--背景创建
function TestScene:Ground()
  self.bg = cc.Sprite:create("buyu2/res/images/Scene/bg1.png")
  ...
  self.Rate =  ccui.Helper:seekWidgetByName(self.ui,"Rate")
...
--其他人子弹的处理
function TestScene:onUserFire(param)
...
function TestScene:onCatchFish(param)
  ...
  if (param.fish_kind >= 18 and param.fish_kind <= 20) or (param.fish_kind >= 24 and param.fish_kind <= 36) or param.fish_kind == 22 or param.fish_kind == 23 or param.fish_kind == 21 then
  ...
  if fish_kind == 21 then
    boodr = false
  elseif fish_kind == 23 or fish_kind == 22 or (fish_kind >= 27 and fish_kind <= 36) then
  ...
  elseif fish_kind >= 27 and fish_kind <= 36 then
    Action:crFishBoom(cc.p(fx,fy))
  ...
  elseif fish_kind >= 24 and fish_kind <= 26 then
    Action:crDsBoom(cc.p(fx,fy))
  else
    Action:crCoin(fish_score,point,self.paopointTable[playerid],fish.rotation)
  end
...
function TestScene:CreateWinScoreItem(score,fishRate)
  ...
  --cclog("winScoreIndex = %d",winScoreIndex)
...
function TestScene:winScoreItemUpdate(dt)
...
function TestScene:GetDieFish(ft,x,y,rot)
...
function TestScene:onBulletIonTimeout(param)
...
function TestScene:onLockTimeout()
  boodr = true
end

--更改鱼的倍率
function TestScene:onHitFishLK(param)
...
--[[
    if v.fishKind == 20 then
      v.rateTex:setS(tostring(v.rate))
    end]]
  end
end

function TestScene:onSwitchScene(param)
  self:switch_scene(param)
end

function TestScene:switch_scene(param)
  self.bolFire = false
  self.scene_style_ = self.scene_style_ or 0
  self.scene_style_ = (self.scene_style_ + 1)%3
  local bg = cc.Sprite:create("buyu2/res/images/Scene/bg" .. (self.scene_style_ + 1) .. ".png")
  --local scalx = winwidth/bg:getContentSize().width
  --local scaly = winheight/bg:getContentSize().height
  -- bg:setAnchorPoint(cc.p(0,0))
  bg:setPosition(winwidth + winwidth / 2,winheight/2)
  --bg:setScaleX(scalx)
  --bg:setScaleY(scaly)
  self:addChild(bg, 4)

  local file = "buyu2/res/images/Scene/wave.png"
  ...
  local function Endcallback()
      ...
      self.bg:setTexture("buyu2/res/images/Scene/bg" .. (self.scene_style_ + 1) .. ".png")
      self.bolFire = true
      soundManager:PlayBackMusic()
  end
  local action = cc.MoveTo:create(6,cc.p(winwidth/2,winheight/2))
...
function TestScene:create_fish_matrix(param)

  if param.scene_kind == 0 then
    for i=0,param.fish_count-1 do
      local trace = FishFactory:get_scene_fish_trace2(0, i)
      BoJFish:creaFish(param.fish_id[i+1], param.fish_kind[i+1], trace,2)
    end
  elseif param.scene_kind == 1 then
    for i=0,param.fish_count-1 do
      local trace = FishFactory:get_scene_fish_trace2(1, i)
      local fish = BoJFish:creaFish(param.fish_id[i+1], param.fish_kind[i+1], trace,2)
      if i < 200 then
        local stop_index, stop_count = FishFactory:get_scene2_small_fish_stop2(i)
        fish:SetFishStop(stop_index, stop_count)
      else
        local stop_index, stop_count = FishFactory:get_scene2_big_fish_stop2()
        fish:SetFishStop(stop_index, stop_count)
      end
    end
  elseif param.scene_kind == 2 then
    for i=0,param.fish_count-1 do
      local trace = FishFactory:get_scene_fish_trace2(2, i)
      BoJFish:creaFish(param.fish_id[i+1], param.fish_kind[i+1], trace,2)
    end
  elseif param.scene_kind == 3 then
    for i=0,param.fish_count-1 do
      local trace = FishFactory:get_scene_fish_trace2(3, i)
      BoJFish:creaFish(param.fish_id[i+1], param.fish_kind[i+1], trace,2)
    end
  elseif param.scene_kind == 4 then
    for i=0,param.fish_count-1 do
      local trace = FishFactory:get_scene_fish_trace2(4, i)
      BoJFish:creaFish(param.fish_id[i+1], param.fish_kind[i+1], trace,2)
    end
  end
end
--[[
function TestScene:Mask()
  self.Mask = Mask.new()
...
function TestScene:GameSetting()
  ...
  self.Setupbtn:setPosition(cc.p(1200,650))
end
...
function TestScene:button()
  ...
  local Redubtn1 = ccui.Button:create("buyu2/res/fishui/Redu.png","buyu2/res/fishui/Redu2.png")
  Redubtn1:setPosition(225-btnX1,winheight-21)
  self:addChild(Redubtn1,7)
  Redubtn1:setVisible(false)
  --加倍率
  local Plusbtn1 = ccui.Button:create("buyu2/res/fishui/Plus.png","buyu2/res/fishui/Plus2.png")
  Plusbtn1:setPosition(378-btnX1,winheight-21)
  self:addChild(Plusbtn1,7)
  Plusbtn1:setVisible(false)
  --减
  local Redubtn2 = ccui.Button:create("buyu2/res/fishui/Redu.png","buyu2/res/fishui/Redu2.png")
  Redubtn2:setPosition(879+btnX2,winheight-21)
  self:addChild(Redubtn2,7)
  Redubtn2:setVisible(false)
  --加
  local Plusbtn2 = ccui.Button:create("buyu2/res/fishui/Plus.png","buyu2/res/fishui/Plus2.png")
  Plusbtn2:setPosition(1033+btnX2,winheight-21)
  self:addChild(Plusbtn2,7)
  Plusbtn2:setVisible(false)

  --减
  local Redubtn3 = ccui.Button:create("buyu2/res/fishui/Redu.png","buyu2/res/fishui/Redu2.png")
  Redubtn3:setPosition(879+btnX2,21)
  self:addChild(Redubtn3,7)
  Redubtn3:setVisible(false)
  --加
  local Plusbtn3 = ccui.Button:create("buyu2/res/fishui/Plus.png","buyu2/res/fishui/Plus2.png")
  Plusbtn3:setPosition(1033+btnX2,21)
  self:addChild(Plusbtn3,7)
  Plusbtn3:setVisible(false)
  --减
  local Redubtn4 = ccui.Button:create("buyu2/res/fishui/Redu.png","buyu2/res/fishui/Redu2.png")
  Redubtn4:setPosition(225-btnX1,21)
  self:addChild(Redubtn4,7)
  Redubtn4:setVisible(false)
  --加
  local Plusbtn4 = ccui.Button:create("buyu2/res/fishui/Plus.png","buyu2/res/fishui/Plus2.png")
  ...
  if SeatNum >= 6 then
    local Redubtn5 = ccui.Button:create("buyu2/res/fishui/Redu.png","buyu2/res/fishui/Redu2.png")
    Redubtn5:setPosition(25,430)
    self:addChild(Redubtn5,7)
    Redubtn5:setRotation(90)
    Redubtn5:setVisible(false)
    --加倍率
    local Plusbtn5 = ccui.Button:create("buyu2/res/fishui/Plus.png","buyu2/res/fishui/Plus2.png")
    Plusbtn5:setPosition(25,280)
    self:addChild(Plusbtn5,7)
    Plusbtn5:setVisible(false)

    local Redubtn6 = ccui.Button:create("buyu2/res/fishui/Redu.png","buyu2/res/fishui/Redu2.png")
    Redubtn6:setPosition(1280-25,300)
    self:addChild(Redubtn6,7)
    Redubtn6:setVisible(false)
    Redubtn6:setRotation(90)
    --加
    local Plusbtn6 = ccui.Button:create("buyu2/res/fishui/Plus.png","buyu2/res/fishui/Plus2.png")
  ...
  if SeatNum == 8 then
    --减
    local Redubtn7 = ccui.Button:create("buyu2/res/fishui/Redu.png","buyu2/res/fishui/Redu2.png")
    Redubtn7:setPosition(640,winheight-21)
    self:addChild(Redubtn7,7)
    Redubtn7:setVisible(false)
    --加
    local Plusbtn7 = ccui.Button:create("buyu2/res/fishui/Plus.png","buyu2/res/fishui/Plus2.png")
    Plusbtn7:setPosition(758,winheight-21)
    self:addChild(Plusbtn7,7)
    Plusbtn7:setVisible(false)

    --减
    local Redubtn8 = ccui.Button:create("buyu2/res/fishui/Redu.png","buyu2/res/fishui/Redu2.png")
    Redubtn8:setPosition(250+390,21)
    self:addChild(Redubtn8,7)
    Redubtn8:setVisible(false)
    --加
    local Plusbtn8 = ccui.Button:create("buyu2/res/fishui/Plus.png","buyu2/res/fishui/Plus2.png")
  ...
  self.Autobtn = ccui.Button:create("buyu2/res/fishui/Auto.png","buyu2/res/fishui/Auto2.png")
  ...
  self.Aimbtn = ccui.Button:create("buyu2/res/fishui/Aim.png","buyu2/res/fishui/Aim2.png")
  ...
  self.Relievebtn = ccui.Button:create("buyu2/res/fishui/Relieve.png","buyu2/res/fishui/Relieve2.png")
...
function TestScene:touchShoot( )
    ...
    if self.bolfd == true and self.is_touch_ then
      if g_score[self.playnum] < self.goma then
        self.bopo = false
      else
        self.bopo = true
      end
...
function TestScene:KeyUpdate(dt)
  --炮管左转
...
--时间函数
function TestScene:Timer()
  if self.schedulerID then return end
...
--[[
    ...
    -- if nowTime - self.currTimer >= 60 then
    --   global.g_mainPlayer:setPlayerScore(math.floor(g_score[num] * g_protion))
    --   WarnTips.showTips(
    --     {
    --       text = "游戏超时,请重新进入游戏!",
...
function TestScene:CreaUi()
  ...
  self.PT1 = cc.Sprite:createWithSpriteFrameName("YQS_Base_00.png")
  ...
  local pRound1 = cc.Sprite:createWithSpriteFrameName("YQS_pRound.png")
  ...
  self.PT2 = cc.Sprite:createWithSpriteFrameName("YQS_Base_00.png")
  ...
  local pRound2 = cc.Sprite:createWithSpriteFrameName("YQS_pRound.png")
  ...
  self.PT3 = cc.Sprite:createWithSpriteFrameName("YQS_Base_00.png")
  self.paoLayer:addChild(self.PT3,3)
  self.PT3:setPosition(cc.p(winwidth-ptX2,40))
  self.PT3:setVisible(false)
  local pRound3 = cc.Sprite:createWithSpriteFrameName("YQS_pRound.png")
  ...
  self.PT4 = cc.Sprite:createWithSpriteFrameName("YQS_Base_00.png")
  self.paoLayer:addChild(self.PT4,3)
  self.PT4:setPosition(cc.p(ptX1,40))
  self.PT4:setVisible(false)
  local pRound4 = cc.Sprite:createWithSpriteFrameName("YQS_pRound.png")
  ...
  if SeatNum >= 6 then
    ...
    self.PT5 = cc.Sprite:createWithSpriteFrameName("YQS_Base_00.png")
    self.paoLayer:addChild(self.PT5,3)
    self.PT5:setPosition(cc.p(40,360))
    self.PT5:setRotation(90)
    self.PT5:setVisible(false)
    local pRound5 = cc.Sprite:createWithSpriteFrameName("YQS_pRound.png")
  ...
  --6
    ...
    self.PT6 = cc.Sprite:createWithSpriteFrameName("YQS_Base_00.png")
    self.paoLayer:addChild(self.PT6,3)
    self.PT6:setPosition(cc.p(1250,360))
    self.PT6:setRotation(-90)
    self.PT6:setVisible(false)
    local pRound6 = cc.Sprite:createWithSpriteFrameName("YQS_pRound.png")
  ...
  --7
    ...
    self.PT7 = cc.Sprite:createWithSpriteFrameName("YQS_Base_00.png")
    ...
    local pRound7 = cc.Sprite:createWithSpriteFrameName("YQS_pRound.png")
  ...
  --8
    ...
    self.PT8 = cc.Sprite:createWithSpriteFrameName("YQS_Base_00.png")
    ...
    local pRound8 = cc.Sprite:createWithSpriteFrameName("YQS_pRound.png")
 ...
 -- self.PlayName3:setPosition(cc.p(380,20))
  ...
  self.honbo1 = cc.Sprite:create("buyu2/res/hobo.png")
  ...
  self.honbo2 = cc.Sprite:create("buyu2/res/hobo.png")
  ...
  self.honbo3 = cc.Sprite:create("buyu2/res/hobo.png")
  ...
  self.honbo4 = cc.Sprite:create("buyu2/res/hobo.png")
  ...
  if SeatNum >= 6 then
    ...
    self.honbo5 = cc.Sprite:create("buyu2/res/hobo.png")
  ...
  ------------------
    ...
    self.honbo6 = cc.Sprite:create("buyu2/res/hobo.png")
  ...
  if SeatNum == 8 then
    ...
    self.honbo7 = cc.Sprite:create("buyu2/res/hobo.png")
  ...
  ---------------------------
    ...
    self.honbo8 = cc.Sprite:create("buyu2/res/hobo.png")
...
--水波形成函数
function TestScene:water()
  self.cache = cc.SpriteFrameCache:getInstance()
  self.cache:addSpriteFrames("buyu2/res/MainScene/Water.plist")
...
function TestScene:layer()
  self.freelayer  =  cc.Sprite:create("buyu2/res/free.png")
  self.freelayer:setPosition(winwidth/2,winheight/2)
  self:addChild(self.freelayer,100)
  self.freelayer:setVisible(false)
  self.freetimer = cc.LabelAtlas:_create("60", "buyu2/res/timer.png",16,32,48)
  self.freetimer:setPosition(555,18)
  self.freelayer:addChild(self.freetimer)
  self.freetimernum = 0
  self.rectime = 60
end

function TestScene:Auto_emission()
  self.actime = 0
  if g_score[self.playnum] > self.min_bullet_multiple then
    self.freetimernum = 0
    self.currTimer = os.time()
