...
--Date   : 2017/4/24
--此文件由[BabeLua]插件自动生成
local GuidedMissileFish = class("GuidedMissileFish",cc.exports.Fish)
function GuidedMissileFish:ctor( fish_kind,  fish_id,  fish_multiple,  fish_speed,  bounding_box_width,  bounding_box_height,  hit_radius,  startPos,  delay)
           GuidedMissileFish.super.ctor(self,fish_kind,  fish_id,  fish_multiple,  fish_speed,  bounding_box_width,  bounding_box_height,  hit_radius,  startPos,  delay);
           ...
           if (self.fis_node) then 
             ...
             local i=0;
			 for  i = 0,32, 1 do
                file_name=string.format("~GuidedMissileFish_000_%03d.png", i);
				local frame =cc.SpriteFrameCache:getInstance():getSpriteFrame(file_name);
				if (frame) then									
						if (cc.exports.g_offsetmap_InitFlag[32]<1) then
                        	local offset_name=string.format("GuidedMissileFish_000_%03d.png", i);
                            ...
                            t_offset_.x =t_x*0.5;--t_offset_.x * 10/ 20;
		 	               t_offset_.y = -t_y*0.5;--t_offset_.y * 10/ 20;   
             ...
             if (readIndex > 0)	then 
                  cc.exports.g_offsetmap_InitFlag[32]=1;
...
function    GuidedMissileFish:GetFish_Catch_Mul()--//获取游戏中鱼的倍数
		local mul =cc.exports.g_pFishGroup:getTotalFish_mul();
		mul =mul+ self:get_fish_mulriple();
		if (mul > 500) then mul = 500; end
		return mul;
end
function  GuidedMissileFish:NetHitTest(bullet)
	if (cc.exports.g_cannon_manager:GetGuideMissileState(bullet:firer_chair_id())>0) then return false; 
	else return self:BulletHitTest(bullet); end
end

function GuidedMissileFish:CatchFish( chair_id,  score,  bulet_mul,  fish_mul,  bomFlag)
...
function  GuidedMissileFish:OnFrame(delta_time)
       ...
       if (self.m_fish_die_action_EndFlag>0) then return true; end
        ...
        if (self.m_catch_fish_Step == 1) then 
            if (self.m_catch_timer > 1.9)	then 
				...
				cc.exports.g_cannon_manager:Catch_GuidedMissile_Card(
...
function GuidedMissileFish:Effect_PlayCallBack()
	self.m_catch_fish_Step = 2;
	--//通知ui获得必杀弹
end

return GuidedMissileFish;
--endregion
