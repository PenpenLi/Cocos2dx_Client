...
--Date   : 2017/4/24
--此文件由[BabeLua]插件自动生成
local FreeGunFish = class("FreeGunFish",cc.exports.Fish)
function FreeGunFish:ctor( fish_kind,  fish_id,  fish_multiple,  fish_speed,  bounding_box_width,  bounding_box_height,  hit_radius,  startPos,  delay)
           FreeGunFish.super.ctor(self,fish_kind,  fish_id,  fish_multiple,  fish_speed,  bounding_box_width,  bounding_box_height,  hit_radius,  startPos,  delay);
          ...
          if (self.fis_node) then 
             ...
             local i=0;
			 for  i = 0,32, 1 do
                file_name=string.format("~FreeGunFish_000_%03d.png", i);
				local frame =cc.SpriteFrameCache:getInstance():getSpriteFrame(file_name);
				if (frame) then									
						if (cc.exports.g_offsetmap_InitFlag[31]<1) then
                        	local offset_name=string.format("FreeGunFish_000_%03d.png", i);
             ...
             if (readIndex > 0)	then 
                  cc.exports.g_offsetmap_InitFlag[31]=1;
...
function FreeGunFish:CatchFish( chair_id,  score,  bulet_mul,  fish_mul,  bomFlag)
    cclog("FreeGunFish:CatchFish( chair_id=%d,  score=%d,  bulet_mul=%d,  fish_mul=%d,  bomFlag=%d)............................................",
    chair_id,  score,  bulet_mul,  fish_mul,  bomFlag);
     local t_chair_id=chair_id;
     self.fis_node:removeChildByTag(7758258);
     self.m_mov_timer = 0;
     self.alive_limi_time=0;
	 self.m_catch_chairID=t_chair_id;
     ...
     self:setLocalZOrder(15);--//激活对应玩家电磁炮 炮塔回飞 跟穿甲弹一样
	  ...
	  if (self.fis_node) then 
       ...
       -- cclog("FreeGunFish:CatchFish( m_catch_chairID=%d,  score,  bulet_mul,  fish_mul,  bomFlag)..........11111111............",self.m_catch_chairID);
end

function  FreeGunFish:OnFrame(delta_time)
       ...
       if (self.m_fish_die_action_EndFlag>0) then return true; end
        ...
        if (self.m_catch_fish_Step == 1) then 
            if (self.m_catch_timer > 1.9)	then 
               ...
               local Alive_pos=self:convertToWorldSpace(cc.p(x,y));
               local alive_angle=self.fis_node:getRotation() + 180;
               
               cc.exports.g_cannon_manager:Catch_FreeBullet_Card( Alive_pos,  alive_angle,  self.m_catch_chairID,  self.m_catch_BulletMul,  self.m_catch_FishMul,  self.m_catch_FishScore)--生成免费子弹

			end
        elseif (self.m_catch_fish_Step == 2) then 
              if (self.m_catch_timer > 0.3) then return true; end
        end
   end
   return false;
end
